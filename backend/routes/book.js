...
router.get("/getbooksbycategory/:categoryId", async (req, res) => {
    try {
        const { categoryId } = req.params; // Changed to categoryId
        const books = await Book.find({ CategoryId: categoryId }); // Use CategoryId in the query
        return res.json({ status: "Success", data: books });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ message: "An Error Occurred" });
